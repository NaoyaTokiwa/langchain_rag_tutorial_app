...
このアプリでは、文書のアップロード、チャンク分割、埋め込み、ベクトルDB保存、検索、回答生成までの流れを、UIで確認しながら体験できます。加えて、検索件数 `k`、プロンプトタイプ、分割方式、会話履歴の有無を切り替えながら、RAGの設計ポイントを比較学習できます。
...
- このアプリでは、さらに会話履歴を持たせることで、前の質問を踏まえた対話型RAGも学べます。
...
- 類似度スコアを確認することで、なぜそのチャンクが選ばれたかを見やすくなります。
...
- 会話履歴ONでは、前の質問と回答を `st.session_state` に保持し、次の質問の文脈補完に使います。
- これにより、単発RAGと対話型RAGの違い、状態管理の重要性、将来的な LangGraph 拡張の考え方を学べます。
...
- **会話履歴あり**: 2つ目の質問が、1つ目の質問内容を踏まえて補完されやすい

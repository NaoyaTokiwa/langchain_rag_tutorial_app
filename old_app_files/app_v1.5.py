...
以下の参考文脈だけを使って、質問への答えを3文以内で短く要約してください。
...
def answer_question(question, k, prompt_type):
    """
    RAGで質問に回答
...
with st.expander("🎓 学習のポイント", expanded=False):
    ...
    - **retrieval_k**: 小→根拠少なめ、大→情報量増加だがノイズも増える
    - **prompt_type**: 回答スタイルを変更
    - **splitter_type**: 分割方式を変更
    - **比較用設定**: 分割方式や chunk_size / chunk_overlap の違いを確認

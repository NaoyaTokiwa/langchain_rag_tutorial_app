...
SESSION_DEFAULTS = {
    ...
    "routing_graph": None,  # Query Routing 用のLangGraph保持用
    "last_route": "",  # Query Routingで選ばれた経路表示用
...
# document ルートでは、既存RAGと同様に質問補完を行う。
# これにより、文書検索が必要な follow-up 質問にも対応しやすくなる。
def routing_rewrite_query_node(state: QueryRoutingState):
    """document ルート向けの検索質問補完ノード"""
    if not state["chat_history"]:
        return {"search_query": state["question"]}
...
def routing_retrieve_node(state: QueryRoutingState):
    """document ルート向けの文書検索ノード"""
    vectorstore = Chroma(
        persist_directory=state["persist_dir"],
        embedding_function=OpenAIEmbeddings(model="text-embedding-3-small"),
    )
...
def routing_generate_document_answer_node(state: QueryRoutingState):
    """document ルート向けの回答生成ノード"""
    history_text = format_chat_history(state["chat_history"], max_turns=3)
    prompt = get_prompt_template(state["prompt_type"])
    llm = ChatOpenAI(model="gpt-4o-mini", temperature=0)
    chain = prompt | llm
...
def routing_web_search_node(state: QueryRoutingState):
    """web ルート向けのWeb調査メモ作成ノード"""
...
def routing_generate_web_answer_node(state: QueryRoutingState):
    """web ルート向けの回答生成ノード"""
    llm = ChatOpenAI(model="gpt-4o-mini", temperature=0)
    history_text = format_chat_history(state["chat_history"], max_turns=3)
...
def routing_general_answer_node(state: QueryRoutingState):
    """general ルート向けの通常応答ノード"""
...
def decide_route_after_classification(state: QueryRoutingState):
...
def tool_calling_finalize_node(state: ToolCallingState):
    """
    Tool Calling後の最終回答をstateに格納する

    Args:
        state: Function Calling用のLangGraph状態

    Returns:
        dict: answerとtool_traceを含む更新状態
    """
...
def build_tool_calling_graph():
    """
    Function Calling学習用のLangGraphを作成する

    Returns:
        CompiledStateGraph: 実行可能なTool Calling用LangGraph
    """
...
def build_query_routing_graph():
    ...
    Returns:
        CompiledStateGraph: 実行可能なQuery Routing用LangGraph
    """
    graph_builder = StateGraph(QueryRoutingState)

    graph_builder.add_node("route_question", route_question_node)
    graph_builder.add_node("rewrite_query", routing_rewrite_query_node)
    graph_builder.add_node("retrieve", routing_retrieve_node)
    graph_builder.add_node(
        "generate_document_answer", routing_generate_document_answer_node
    )
    graph_builder.add_node("web_search", routing_web_search_node)
    graph_builder.add_node("generate_web_answer", routing_generate_web_answer_node)
...
def answer_question_with_query_routing(question, k, prompt_type, chat_history=None):
    ...
    result = st.session_state.routing_graph.invoke(initial_state)
...
def reset_chat_history():
    ...
    st.session_state.routing_graph = None
...
# ルーティング設計の意図を理解しやすい。
def render_query_routing_view():
    """Query Routingの選択結果を表示する"""
    if not st.session_state.last_route:
        return
    ...
    st.info(f"選択された経路: **{selected_route_label}**")

    if st.session_state.last_route == "web" and st.session_state.last_web_context:
...
def handle_answer_generation(question):
    ...
    try:
        with st.spinner("検索中...回答生成中..."):
            ...
            elif st.session_state.execution_mode == "Query Routing RAG":
                answer, route, retrieved_results, web_context = (
                    answer_question_with_query_routing(
                        question,
...
with right_col:
    ...
    execution_mode = st.radio(
        ...
        help=(
            ...
            "Query Routing RAGは質問内容ごとに最適な経路へ分岐します。"
        ),
    )
